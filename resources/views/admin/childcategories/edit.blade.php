<x-app-layout>
    ...
    <div class="py-10">
        ...
        <div class="container mx-auto shadow sm:rounded-md">
            <form class="p-5" enctype="multipart/form-data" action="{{ route('childcategories.update', $childcategory->id)}}" method="POST">
                @csrf
                @method('PUT')
                <div class="mb-6">
                ...
                <div class="mb-6">
                    <label for="subcategory" class="block mb-2 text-sm font-medium text-gray-900"> Sub Category:</label>
                    <select class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-black focus:border-black block w-full p-2.5"
                        ...
                        @foreach (App\Models\SubCategory::all() as $subcategory)
                            <option value="{{ $subcategory->id }}" {{$subcategory->id == $childcategory -> sub_category_id ? 'selected' : ''}}>
                                {{ $subcategory->name}}
                            </option> 
                        @endforeach
                    </select>
                ...
                <div class="mb-6">
                    ...
                    <div class="flex items-center">
                        <img class="w-10 h-10 mb-2 rounded-full" src="{{ Storage::url( $childcategory->image )}}">
                    </div>
